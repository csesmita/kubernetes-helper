...
def setup(is_central, num_sch, misestimation):
    ...
    for row in f:
        ...
        #Populate a mis-estimated value
        sigma = misestimation * est_time
        while True:
            est_time = random.gauss(est_time, sigma)
            if est_time > 0:
                break
        actual_duration = []
...
def main():
    ...
    if len(sys.argv) != 5:
        print("Incorrect number of parameters. Usage: python jobs.py d 10 0.5 -e")
        sys.exit(1)
    ...
    misestimation = float(sys.argv[3])
    max_job_id = setup(is_central, num_sch, misestimation)
