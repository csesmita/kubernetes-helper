...
ax[0].plot(std_queue_add_time, label="Added to scheduler's queue", marker="o", markersize=2, markeredgecolor=colors[0], markerfacecolor=colors[0], color=colors[0])
legend = ax[0].legend(loc='upper center')
print("Plotting", std_queue_delete_time)
ax[1].plot(std_queue_delete_time, label="Scheduled", marker="o", markersize=2, markeredgecolor=colors[1], markerfacecolor=colors[1], color=colors[1])
legend = ax[1].legend(loc="upper center")
print("Plotting", std_execution_start_time)
ax[2].plot(std_execution_start_time, label="Execution started", marker="o", markersize=2, markeredgecolor=colors[2], markerfacecolor=colors[2], color=colors[2])
legend = ax[2].legend(loc="upper center")
print("Plotting", std_pod_done_time)
ax[3].plot(std_pod_done_time, label="Execution ended", marker="o", markersize=2, markeredgecolor=colors[3], markerfacecolor=colors[3], color=colors[3])
legend = ax[3].legend(loc="upper center")
fig.text(0.5, 0.00, 'JobIDs', ha='center')
fig.text(0.00, 0.5, 'Standard Deviation (s)', va='center', rotation='vertical')
#xticks = np.arange(0, 10003, 2000)
delta = max(RANDOM_SAMPLE/5, 1)
xticks = np.arange(0, RANDOM_SAMPLE + 2, delta)
ax[3].set_xticks(xticks)
fig.savefig('point5_all_1000.pdf', dpi=fig.dpi, bbox_inches='tight')
