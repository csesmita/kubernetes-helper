...
def setup(is_central, num_sch):
    ...
    for row in f:
        ...
        if not is_central:
            ...
            filedata = job_tmpl.replace('$JOBID',jobstr).replace("$NUM_TASKS",str(num_tasks)).replace("$SCHEDULER_NAME", schedulername).replace("$ESTRUNTIME", str(est_time)).replace("$JOBPRIO",str(jobid))
        else:
            filedata = job_tmpl.replace('$JOBID',jobstr).replace("$NUM_TASKS",str(num_tasks)).replace("$ESTRUNTIME", str(est_time)).replace("$JOBPRIO",str(jobid))
