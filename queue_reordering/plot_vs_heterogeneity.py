...
dirname='/home/sv440/Android/eagle/simulation/results_new/queue_reordering/1000X/10S/slots=0'
jct_per_setup = defaultdict(list)
...
for filename in os.listdir(dirname):
    ...
    jct_per_setup[index].append(jct)
...
for index in range(5):
    mean = np.mean(jct_per_setup[index], axis=0)
    std = np.std(jct_per_setup[index], axis=0)
    x = range(len(mean))
...
fig.savefig('reordering_vs_setup_homogeneous.pdf', dpi=fig.dpi, bbox_inches='tight')
