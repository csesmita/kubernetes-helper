...
for index in range(5):
    mean = []
    std = []
    for num_sched_dir in jct_99p_schemes_per_scheduler.keys():
        mean.append(np.mean(jct_99p_schemes_per_scheduler[num_sched_dir][index]))
        std.append(np.std(jct_99p_schemes_per_scheduler[num_sched_dir][index]))
    #print(mean, std)
...
fig.savefig('reordering_99p_vs_schedulers.pdf', dpi=fig.dpi, bbox_inches='tight')
