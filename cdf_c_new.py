...
c = np.sort(c_xt_list)
cp = 1. * np.arange(len(c)) / (len(c) - 1)
ax_jct.plot(c, cp, label="Average x", linestyle=':', color=colors[2], linewidth=3)
...
ax_jct.plot(c, cp, label="Average TST", linestyle='-.', color=colors[0], linewidth=3)
...
ax_jct.plot(c, cp, label="Average TCT", color=colors[1], alpha=0.5, linewidth=3)
c = np.sort(t_c_sq_list)
cp = 1. * np.arange(len(c)) / (len(c) - 1)
ax_jct.plot(c, cp, label="Tail TST", linestyle='--', color=colors[0], linewidth=3)
...
ax_jct.plot(c, cp, label="Tail TCT", color=colors[1], alpha=0.5, linestyle='--', linewidth=3)
...
ax_tail.set_ylim(0, 17500)
ax_tail.set_ylabel('CPU (cores)', color=colors[0])
ax_tail.set_yticklabels(["0", "5", "10", "15"])
#time, tail latency
...
ax_tail2.set_ylabel('Tail $S_t$ (1000 s)', color=colors[1])
ax_tail2.set_ylim(0, 50000)
ax_tail2.set_yticks(np.arange(0, 60000 , 10000))    
ax_tail2.set_yticklabels(["0", "10", "20", "30", "40", "50"])    
...
legend = plt.legend([l1, l2], ["Average CPU utilization on nodes", "99th percentile tail $S_t$"], framealpha=0.5, prop={'size': 11})
frame = legend.get_frame()
frame.set_facecolor('1.0')
frame.set_edgecolor('1.0')
fig.tight_layout()
...
fig.tight_layout()
#fig.savefig('c_tail_tc_util.pdf', dpi=fig.dpi, bbox_inches='tight')
fig.savefig('c_d_tail_tc_yaqc.pdf', dpi=fig.dpi, bbox_inches='tight')

###Murmuration Scheduler
# Scheduler Algorithm and Queue times, and the execution times.
tct = []
w2x = []
tail_tct = []
...
ax_tail.set_ylabel('CDF')
fig.tight_layout()
fig.savefig('c_d_tail_tc_murmuration.pdf', dpi=fig.dpi, bbox_inches='tight')
